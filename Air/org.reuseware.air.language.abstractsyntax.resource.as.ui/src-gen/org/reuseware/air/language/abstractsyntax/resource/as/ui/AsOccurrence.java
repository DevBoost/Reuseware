...
public class AsOccurrence {
	...
	private org.reuseware.air.language.abstractsyntax.resource.as.ui.AsTokenScanner tokenScanner;
	...
	private org.reuseware.air.language.abstractsyntax.resource.as.IAsTextResource textResource;
	 ...
	 */
	public AsOccurrence(org.reuseware.air.language.abstractsyntax.resource.as.IAsTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.reuseware.air.language.abstractsyntax.resource.as.ui.AsTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.reuseware.air.language.abstractsyntax.resource.as.mopp.AsMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.reuseware.air.language.abstractsyntax.resource.as.ui.AsBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addPosition(document, org.reuseware.air.language.abstractsyntax.resource.as.ui.AsPositionCategory.DEFINTION.toString());
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addPosition(document, org.reuseware.air.language.abstractsyntax.resource.as.ui.AsPositionCategory.PROXY.toString());
					}
				}
			}
			token = tokenScanner.nextToken();
